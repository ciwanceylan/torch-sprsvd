...
@pytest.mark.parametrize("mode", ['col_projection', 'row_projection'])
def test_halko_rsvd(type_: int, k: int, dtype: torch.dtype, mat_type: str, mode: tsprsvd.HALKO_RSVD_MODES):
    ...
    for i, num_oversampling in enumerate([0, 5, 10, 20]):
        U, sig_values, Vh = tsprsvd.single_pass_rsvd(input_matrix=input_matrix, k=k,
